...
/**
 * test for valid number of tokens in a row for validation of user input
 **/
#define VALID_NUM_IN_ROW(N) (((N) >= MININROW) && ((N) <= MAXINROW))
 ...
 * of what these do, please see the rules.c file.
 **/
BOOLEAN valid_move(struct game*, const struct coordinate*);
