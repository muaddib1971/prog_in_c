...
and libasan to help you in this task. These tools were
covered in a previous lab, so look up those notes if you are
struggling with this. While you are working through this
task, keep notes on what you are doing so that you can record 
details in the google form when you get the assessme itself.
...
The following targest are also available in the Makefile: 

make debug: compiles the project with debugging symbols 
built in so you can debug the program using gdb and valgrind. 

make sanitize: compiles the program with libasan. Please note you'll
also in this case need to ensure you are using a more modern compiler
such as gcc 8 by running the following command first: 

scl enable devtoolset-8 bash

You are able to use resources online, those available in
canvas and elsewhere. The only limitation is that you
